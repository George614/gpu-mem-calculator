...
def calculate(config: str, output: str | None, format: Literal["json", "yaml", "table"]) -> None:
    """Calculate GPU memory requirements from config file.

    Example:
        gpu-mem-calc calculate --config configs/llama2_7b.json
    """
    if not config:
        click.echo("Error: --config is required for calculate command", err=True)
        sys.exit(1)
